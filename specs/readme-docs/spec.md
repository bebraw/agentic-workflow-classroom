...
The README is the first surface contributors and lecturers see. It should identify the current classroom demo clearly near the top, explain how vendored ASDLC references relate to repo-specific docs, point contributors at the current runtime and verification commands, and keep screenshot refresh lightweight and local-only.

### Architecture

- **Primary document:** `README.md`
- **Committed screenshot asset:** `docs/screenshots/home.png`
- **Current workflow summary:** quick demo, runtime, verification, source layout, and documentation contract notes in `README.md`
- **Screenshot refresh path:** `npm run screenshot:home`
- **Update model:** local scripted refresh when the classroom UI changes materially
...
- Do not let the committed screenshot drift far from the current classroom UI.
...
- Do not let the README drift away from the actual commands, ports, or source layout used by the current classroom demo.
...
- [ ] The README identifies the classroom demo as a Cloudflare Worker served with Wrangler near the top.
...
- `README.md` should let a new reader understand the current classroom demo and rendering model before they start exploring the source tree.
...
- `README.md` should continue to describe the current classroom source layout and verification flow accurately.
...
- The screenshot should continue to represent the current classroom app surface closely enough to be useful.
...
- Then: they can tell quickly that the classroom demo is a Cloudflare Worker served with Wrangler and centered on server-rendered HTML

**Scenario: Contributor follows the README**

- Given: the current classroom demo baseline
...
**Scenario: Classroom UI changes materially**
